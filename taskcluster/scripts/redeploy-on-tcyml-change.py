...
def deploy_token(app, repo):
    """A short-lived github token that may dispatch the deploy workflow."""
    owner, name = repo.split("/", 1)
    auth = taskcluster.Auth({"rootUrl": os.environ["TASKCLUSTER_PROXY_URL"]})
    return auth.githubRepoToken(
        app,
        owner,
        {"repositories": [name], "permissions": {"actions": "write"}},
    )["token"]
...
def parse_args(args=None):
    ...
    parser.add_argument(
        "--github-app",
        required=True,
        help="Auth service application to mint the dispatch token with",
    )
...
def main(args=None):
    ...
    token = deploy_token(args.github_app, args.deploy_repo)
    dispatch_deploy(token, args.deploy_repo, args.deploy_workflow, args.deploy_ref)
