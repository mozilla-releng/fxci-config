...
async def get_ciconfig_dir(dirname):
    ...
    async with _lock.setdefault(cache_key, Lock()):
        ...
        if other_files:
            raise ValueError(
                f"Files in directory {dirname} contain unsupported types (not dict or list): {[of[0] for of in other_files]}"
            )

        if dict_files and list_files:
            raise ValueError(
                f"Directory {dirname} contains mixed file types (both dicts and lists)"
            )
        ...
        if dict_files:
            ...
            for filename, content in dict_files:
                for key, value in content.items():
                    if key in result:
                        raise ValueError(
                            f"Duplicate key '{key}' found in files {key_to_file[key]} and {filename}"
                        )
                    result[key] = value
